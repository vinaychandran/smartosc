...
const throwError = message => window.console.error(`popup: ${message}`);
...
class popup {
    constructor(id, config) {
        ...
        this.modal = popup.findModal(this.id);

        this.dialog = this.modal.querySelector(`.${simpleModalClass.DIALOG}`);
        ...
        this.triggers = popup.findTriggers(this.id);
        this.closeEls = null;
    ...
    open(e) {
        ...
        if (this.config.transition) {
            this.modal.classList.add(simpleModalClass.OPEN);
            this.modal.classList.add(simpleModalClass.IN);
    ...
    close() {
        ...
        if (this.config.transition) {
            this.modal.classList.remove(simpleModalClass.IN);
            this.modal.classList.remove(simpleModalClass.OPEN);
            this.modal.classList.add(simpleModalClass.OUT);
    ...
    closeWithTransition() {
        const closeTransitionHandler = () => {
            ...
            this.modal.style.display = 'none';
            this.modal.classList.remove(simpleModalClass.OUT);
    ...
    showBackdrop() {
        ...
        this.backdrop.classList.add(simpleModalClass.BACKDROP);

        if (this.config.transition) {
            this.backdrop.classList.add(simpleModalClass.IN);
        }
        ...
        this.backdrop.classList.add(simpleModalClass.OPEN);
    }

    closeBackdrop() {
        if (this.config.transition) {
            this.backdrop.classList.remove(simpleModalClass.IN);
            this.backdrop.classList.remove(simpleModalClass.OPEN);
            this.backdrop.classList.add(simpleModalClass.OUT);
